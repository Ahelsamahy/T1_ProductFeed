...
mytree = ET.parse('202208112017.xml')
...
for index in range(0, recordLen):
    # print (myroot[index][4].text)
    myroot[index][4].text = str("https://butopea.com/p/" + myroot[index][4].text)
    if len(myroot[index][5].text)>2: 
      myroot[index][5].text = str("https://butopea.com/p/" + myroot[index][5].text)
...
# print(len(myroot.findall('DATA_RECORD')))
...
# for links in myroot.iter('image_link'):
#     # updates the price value
#     links.text = str("https://butopea.com/"+(links.text))
#     counter += 1
# print("changed image_link for",counter, "product")
# counter = 0
